      ...
      <div className="dashboard">
        ...
        <div className="top-grid">
          ...
          <Panel><PanelTitle icon={<Newspaper/>} title="Capital flow" subtitle="Follow the money across markets"/>
            ...
            <div className="flow-update"><div className="update-head"><span>↗ Last Update 21:00</span><b>● Live</b></div>{marketBars.map(([name,val,num], index)=><div className="bar-row" key={`${name}-${index}`}><strong>{name}</strong><div><i className={val>=0?"positive":"negative"} style={{width:`${Math.abs(val)}%`}}/></div><span className={val>=0?"gain":"loss"}>{num}</span></div>)}</div>
          </Panel>
        </div>
